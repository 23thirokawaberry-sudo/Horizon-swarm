...
var weapon_upgrade = [[[192, 0], "Shotgun unlock", 4], [[224, 32], "Pistol unlock", 5]] #Weapon upgrades - each weapon has a certain max level, after which they no longer can be upgraded.
var unlocked_weapon = [["Shotgun", 0], ["Sword", 0], ["Beam", 0], ["Pistol", 0]] #counts how many times the weapon has been upgraded, before removing it from the available list(s).
...
var available_limit = [2, 1] #each position in this list relates to the highest possible value in their indivisual lists.
